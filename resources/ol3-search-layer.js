    ...
    var returnHorsey = function(input, source, map, select, options) {
      horsey(input, {
        ...
        predictNextSearch: function(info) {
          // Funkcja obliczająca dopasowanie do wpisanej frazy
          function getMatchScore(searchText, featureText) {
            const regex = new RegExp(searchText, 'i'); // Ignorowanie wielkości liter
            return (featureText.match(regex) || []).length;
          }

          // Wyszukaj wszystkie wyniki
          const featuresList = source.getFeatures().map(function(el) {
            return {
              feature: el,
              score: getMatchScore(info.selection.value, el.get(options.colName)) // Dopasowanie
            };
          });

          // Posortuj wyniki na podstawie dopasowania (malejąco)
          featuresList.sort(function(a, b) {
            return b.score - a.score; // Większe dopasowanie ma wyższą pozycję
          });

          // Przeładuj posortowaną listę na nowo
          const sortedList = featuresList.map(function(item) {
            return {
              text: item.feature.get(options.colName),
              value: item.feature.getId()
            };
          });

          // Zaktualizuj horsey listę
          horsey(input, {
            source: [{
              list: sortedList
            }],
